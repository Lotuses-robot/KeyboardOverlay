  ...
  const refreshOverallInfo = () => {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        for (var i in data) {
          // setErrorMessage(`Debu: ${data[i].key}`)
          keyCounter[data[i].key.toUpperCase()] = data[i].cnt;
          sum += data[i].cnt;
        }
        hitTotal = sum;
      },
      error: function () {
        flushDataFile();
        // setErrorMessage("Please restart.");
      }
    })
      ...
      success: function (data) {
        ...
        for (var i = 0; i < data.length; i++) {
          ...
          for (var j = 0; j < p.length; j++) {
            ...
            if (dt[0] == "<Row>")
              ...
              var ht = getNum(dt[3]);
              var ft = getNum(dt[4]);
              res += `<div class="buttonDiv" key="${keyIdMask(cnt)}" height=${ht} width=${wd} font=${ft}><span>${idToKey[cnt].name}</span>${keyCount ? `<span class='counter'>${keyCounter[cnt] ? keyCounter[cnt] : 0}</span>` : ""}</div>`;
              if (!keyCounter[cnt]) {
                keyCounter[cnt] = 0;
              }
              // setErrorMessage(`debug ${cnt}`);
              // return;
            }
            else if (dt[0] == "<Icon>") {
              if (dt.length === 1) {
              ...
              var ht = getNum(dt[3]);
              var ft = getNum(dt[4]);
              res += `<div class="buttonDiv" key="${keyIdMask(cnt)}" height=${ht} width=${wd} font=${ft}><span>${icons[cnt]}</span>${keyCount ? `<span class='counter'>${keyCounter[cnt] ? keyCounter[cnt] : 0}</span>` : ""}</div>`;
              if (!keyCounter[cnt]) {
                keyCounter[cnt] = 0;
              }
              // setErrorMessage(`debug ${cnt}`);
        ...
        $(".keyboardContainer").html("<div style='margin-bottom: 2px; overflow: visible'>"
          + (toolBarMode === "none" ? "" : "<div class='debugInfo'>" +
            (toolBarMode === "kps" ? (keyTotalCountMode === "normal" ? "" : "S-") + `TOTAL ${hitTotal}<span style='position: absolute; top: 0px; right: 0px'>KPS 0</span>` : (toolBarMode === "tot" ? (keyTotalCountMode === "normal" ? "" : "S-") + `TOTAL ${hitTotal}` : "DEBUG INFO"))
            + "</div>")
          + res
          + "</div>");
        $(".buttonDiv").each(function () {
        ...
        for (var i in recvData) {
          flushHeatmap(recvData[i].key.toUpperCase());
        }
        flushDataFile();
        if (errorMessage)
          return;
